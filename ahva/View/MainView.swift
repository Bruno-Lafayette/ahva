        ...
        if networkMonitor.isConnected{
            ...
            NavigationView{
                ScrollView {
                    LazyVGrid(columns: columns){
                        ForEach(styles, id: \.self) { style in
                            NavigationLink {
                                AvatarView(style,
                                           seedDefault,
                                           url: "https://api.dicebear.com/5.x/\(style)/png?\(key)=\(seedDefault)")
                            } label: {
                                StyleCell(title: style,
                                          imageURL: "https://api.dicebear.com/5.x/\(style)/png?\(key)=\(seedDefault)",
                                          text: true)
                            }
                    ...
                    .task {
                        do {
                            let request = try HTTPRequestFactory(path: "https://databaseavatar.vercel.app/api/style",
                                                                 method: .GET).createRequest()
